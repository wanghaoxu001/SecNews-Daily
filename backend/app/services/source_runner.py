...
def _event(step: str, status: str, message: str, detail: str | None = None) -> dict:
...
async def run_source_pipeline(db: AsyncSession, source: RssSource) -> AsyncGenerator[dict, None]:
    ...
    try:
        ...
        yield _event("fetch", "success", f"抓取完成，新增 {new_count} 篇文章")
    except Exception as e:
        yield _event("fetch", "error", f"RSS 抓取失败: {e}", traceback.format_exc())
    ...
    for idx, news in enumerate(pending_news, 1):
        ...
        try:
            async for progress_msg in process_single_news_gen(db, news):
                yield _event("process", "info", f"{prefix} {progress_msg}")

            if news.process_status == ProcessStatus.failed.value:
                processed_fail += 1
                yield _event("process", "error", f"{prefix} 处理失败", news.process_error)
                continue
        except Exception as e:
            processed_fail += 1
            yield _event("process", "error", f"{prefix} 处理异常: {e}", traceback.format_exc())
            continue

        yield _event("process", "success", f"{prefix} 基础处理完成")
        ...
        try:
            ...
            yield _event("similarity", "success", f"{prefix} 相似性检查完成: {sim_msg}")
        except Exception as e:
            yield _event("similarity", "error", f"{prefix} 相似性检查失败: {e}", traceback.format_exc())
        ...
        try:
            ...
            yield _event("importance", "success", f"{prefix} 重要性判断完成: {imp_msg}")
        except Exception as e:
            yield _event("importance", "error", f"{prefix} 重要性判断失败: {e}", traceback.format_exc())
